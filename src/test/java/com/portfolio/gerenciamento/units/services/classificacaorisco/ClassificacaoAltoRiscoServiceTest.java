...
class ClassificacaoAltoRiscoServiceTest {
    ...
    private static final int LIMITE_MESES_ALTO_RISCO = 6;

    private final ClassificacaoAltoRiscoService service = new ClassificacaoAltoRiscoService(defaultProperties());

    @DisplayName("Deve atender regra de alto risco quando orcamento ou duracao ultrapassar limite")
    @ParameterizedTest(name = "orcamento={0}, meses={1}")
    @CsvSource({
            "500000.01, 1",
            "50000, 7"
    })
    void should_returnTrue_when_budgetOrDurationExceedsHighRiskLimit(BigDecimal orcamento, int meses) {
    ...
    @Test
    @DisplayName("Deve nao atender regra de alto risco quando orcamento e duracao estiverem no limite")
    void should_returnFalse_when_budgetAndDurationAreWithinHighRiskLimit() {
        Projeto projeto = projeto(LIMITE_ORCAMENTO_ALTO_RISCO, LIMITE_MESES_ALTO_RISCO);
    ...
    @Test
    @DisplayName("Deve retornar classificacao alto quando consultar classificacao da regra")
    void should_returnHighRiskClassification_when_classificationIsRequested() {
        ClassificacaoRisco classificacao = service.classificacao();

        assertThat(classificacao).isEqualTo(ClassificacaoRisco.ALTO);
    }
